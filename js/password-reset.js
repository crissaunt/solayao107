            ...
            if (data.message.toLowerCase().includes('email not found') || 
                ...
                setTimeout(() => {
                    emailError.innerHTML = data.message + '<br><br>' +
                        ...
                        '<button onclick="window.location.href=\'../html/register.html\'" style="background: #4CAF50; color: white; border: none; padding: 8px 16px; border-radius: 4px; cursor: pointer;">Create new account</button>';
                }, 100);
            }
        }
    })
